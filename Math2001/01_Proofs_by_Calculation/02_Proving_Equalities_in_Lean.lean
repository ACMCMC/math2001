...
example {a b m n : ℤ} (h1 : a * m + b * n = 1) (h2 : b ^ 2 = 2 * a ^ 2) :
  ...
  calc
    (2 * a * n + b * m) ^ 2
    _ = 4 * a^2 * n^2 + (b ^ 2) * m^2 + 4 * a * n * b * m := by ring
    _ = 4 * a^2 * n^2 + (2 * a ^ 2) * m^2 + 4 * a * n * b * m := by rw[h2]
    _ = 2 * (2 * a^2) * n^2 + 2 * a ^ 2 * m^2 + 4 * a * n * b * m := by ring
    _ = 2 * (b ^ 2) * n^2 + b^2 * m^2 + 4 * a * n * b * m := by rw[h2]
    _ = 2 * (b ^ 2) * n^2 + (2 * a^2) * m^2 + 4 * a * n * b * m := by rw[h2]
    _ = 2 * ((a * m)^2 + (b * n)^2) + 2 * 2 * a * n * b * m := by ring
    _ = 2 * ((a * m)^2 + (b * n)^2 + 2 * a * m * b * n) := by ring
